...
module.exports = {
  ...
  run: async(client, message, args) => {
    ...
    const pngAv = Target.displayAvatarURL({ format: 'png', size: 1024 }) || DefaultAv;
    const jpgAv = Target.displayAvatarURL({ format: 'jpg', size: 1024 }) || DefaultAv;
    const dynaAv = Target.displayAvatarURL({ size: 1024, dynamic: true }) || DefaultAv;
    const webpAv = Target.displayAvatarURL({ format: 'webp', size: 1024 }) || DefaultAv; 

    let Flags = Target.flags.toArray().join(' ');
    if(!Flags){
    ...
    const userInfoEmbed = new MessageEmbed()
      ...
      .setAuthor(`${Target.tag}`, Target.displayAvatarURL({dynamic: true}) || DefaultAv)
      .setColor(Target.displayHexColor)
      .setThumbnail(Target.displayAvatarURL({ size: 1024, dynamic: true }) || DefaultAv)
